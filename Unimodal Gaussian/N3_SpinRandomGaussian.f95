PROGRAM SpinRandomField_N3
   ...
   DO WHILE (H <= 10)
      ...
      result = result / REAL(eval_num*3.D0, kind = 8 )

      WRITE(24, *)  H, result                                          ! MAGNETIZAÇÃO MÉDIA POR SPIN E POR DESORDEM VS CAMPO MAGNÉTICO EXTERNO
...
SUBROUTINE Energy(S, X, dim_num, Ns, H, Energia)
   ...
   DO j = 1, 2**Ns  ! LOOP DAS CONFIGURAÇÕES DE SPIN
      ...
      Energia(j) = S(1, j) * S(2, j) + S(1, j) * S(3, j) + S(2, j) * S(3, j) - H * SUM(S(:, j)) &
         - X(1)*S(1, j) + X(2)*S(2, j) + X(3)*S(3, j)
