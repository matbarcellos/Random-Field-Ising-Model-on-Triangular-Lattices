PROGRAM SpinRandomField_N3
   ...
   DO WHILE (H <= 10)
      ...
      result = result / REAL(eval_num*9.D0, kind = 8 )

      WRITE(26, *)  H, result                                          ! MAGNETIZAÇÃO MÉDIA POR SPIN E POR DESORDEM VS CAMPO MAGNÉTICO EXTERNO
...
SUBROUTINE Energy(S, x, dim_num, Ns, H, Energia)
   ...
   DO j = 1, 2**Ns  ! LOOP DAS CONFIGURAÇÕES DE SPIN
      ...
      Energia(j) = S(1, j)*S(2, j) + S(2, j)*S(3, j) + S(3, j)*S(4, j) + S(4, j)*S(5, j) + S(5, j)*S(6, j) + S(6, j)*S(7, j)  &
         + S(7, j)*S(8, j) + S(8, j)*S(9, j) + S(8, j)*S(1, j) + S(2, j)*S(8, j) + S(2, j)*S(9, j) + S(3, j)*S(9, j)          &
         + S(4, j)*S(9, j) + S(4, j)*S(6, j) + S(6, j)*S(9, j) + S(7, j)*S(9, j) - H * SUM(S( : , j))                         &
         - x(1)*S(1, j) - x(2)*S(2, j) - x(3)*S(3, j) - x(4)*S(4, j) - x(5)*S(5, j) - x(6)*S(6, j) - x(7)*S(7, j)             &
         - x(8)*S(8, j) - x(9)*S(9, j)
   END DO
