PROGRAM CLUSTER_BIMODAL
    ...
    REAL(8) :: H, H_0
    REAL(8) :: SpinInteraction(2**N), X(3**N)
    ...
    call BaseCampos(H_0, R, N)     ! BASE DE CAMPOS ALEATÓRIOS
 ...
 SUBROUTINE Susceptibilidade(S, R, N, H, SpinInteraction, X, T)
    ...
    REAL(8) :: S(N, 2**N), R(N, 3**N), Energia(3**N, 2**N), Z(3**N), M(3**N), M_d(3**N), M_l(3**N), T
    REAL(8) :: X(3**N), SpinInteraction(2**N)
    REAL(8) :: H, H_d, H_l, dh
